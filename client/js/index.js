...
function UpdateGui(upsStatus) {
  ...
  document.getElementById('barOutputLoad').style.width = `${upsStatus.outputLoad}%`;
  document.getElementById('barOutputLoad').setAttribute('aria-valuenow', upsStatus.outputLoad);
  ...
  document.getElementById('barSoc').style.width = `${upsStatus.soc}%`;
  document.getElementById('barSoc').setAttribute('aria-valuenow', upsStatus.soc);
  document.getElementById('fieldBoardTemperature').value = upsStatus.ucTemperature;
  document.getElementById('checkDevCharging').classList.toggle('active', !!(upsStatus.deviceStatus & 0x01));
  document.getElementById('checkDevDischarging').classList.toggle('active', !!(upsStatus.deviceStatus & 0x02));
  document.getElementById('checkDevPowerPresent').classList.toggle('active', !!(upsStatus.deviceStatus & 0x04));
  document.getElementById('checkDevBatteryPresent').classList.toggle('active', !!(upsStatus.deviceStatus & 0x08));
  document.getElementById('checkDevShutdownSet').classList.toggle('active', !!(upsStatus.deviceStatus & 0x10));
  document.getElementById('checkDevOverCurrent').classList.toggle('active', !!(upsStatus.deviceStatus & 0x20));
  ...
  document.getElementById('checkCharging').classList.toggle('active', !!(upsStatus.chargeStatus & 0x01));
  document.getElementById('checkBackup').classList.toggle('active', !!(upsStatus.chargeStatus & 0x02));
  document.getElementById('checkConstantVoltage').classList.toggle('active', !!(upsStatus.chargeStatus & 0x04));
  document.getElementById('checkConstantCurrent').classList.toggle('active', !!(upsStatus.chargeStatus & 0x08));
  document.getElementById('checkUnderVoltage').classList.toggle('active', !!(upsStatus.chargeStatus & 0x10));
  document.getElementById('checkInputCurrentLimit').classList.toggle('active', !!(upsStatus.chargeStatus & 0x20));
  document.getElementById('checkCapacitorPowerGood').classList.toggle('active', !!(upsStatus.chargeStatus & 0x40));
  document.getElementById('checkCapacitorShunting').classList.toggle('active', !!(upsStatus.chargeStatus & 0x80));
  document.getElementById('checkCapacitorBalancing').classList.toggle('active', !!(upsStatus.chargeStatus & 0x100));
  document.getElementById('checkChargeDisabled').classList.toggle('active', !!(upsStatus.chargeStatus & 0x200));
  document.getElementById('checkInputPowerFail').classList.toggle('active', !!(upsStatus.chargeStatus & 0x800));
  document.getElementById('checkCapEsrMeasurement').classList.toggle('active', !!(upsStatus.monitorStatus & 0x01));
  document.getElementById('checkCapEsrWaitingTime').classList.toggle('active', !!(upsStatus.monitorStatus & 0x02));
  document.getElementById('checkCapEsrWaitingCondition').classList.toggle('active', !!(upsStatus.monitorStatus & 0x04));
  document.getElementById('checkCapMeasurementComplete').classList.toggle('active', !!(upsStatus.monitorStatus & 0x08));
  document.getElementById('checkEsrMeasurementComplete').classList.toggle('active', !!(upsStatus.monitorStatus & 0x10));
  document.getElementById('checkCapMeasurementFail').classList.toggle('active', !!(upsStatus.monitorStatus & 0x20));
  document.getElementById('checkEsrMeasurementFail').classList.toggle('active', !!(upsStatus.monitorStatus & 0x40));
  document.getElementById('checkChargerDisabled').classList.toggle('active', !!(upsStatus.monitorStatus & 0x100));
  document.getElementById('checkChargerEnabled').classList.toggle('active', !!(upsStatus.monitorStatus & 0x200));
